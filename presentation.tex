...
\begin{frame}
  ...
  \begin{itemize}
    \item The general way to simulate $H_{\mathbf{X}}$ is to use \emph{Trotterization}\cite{lloyd_1996}
    \begin{itemize}
      \item \emph{Requirement}: we need to know how to decompose $H_{\mathbf{X}}$ in terms that are \emph{easy to simulate}.
      \item We can always decompose an Hermitian matrix in $O(MN)$ Pauli strings.
    \end{itemize}
    \item More efficient algorithms exist for \emph{sparse} Hamiltonians, but they require an \emph{oracle}
    \begin{itemize}
      \item Row $i$, $j$th non-zero element
      \[\ket{i,j,0}\rightarrow \ket{i,j,H_{ij}}\]
      \item Likely need to hard-code it and full of multi-controlled gates
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  where the frequencies are completely determined by the eigenvalues of the data-encoding Hamiltonian
...
\begin{frame}
  ...
  \ \\
  For instance, if two of the features are the same, then $x_i=x_j$
  \[e^{in_i x_i}e^{in_j x_j} = e^{i2n x}\] 

  \ \\
  From this observation derives the \emph{universality} of quantum models with time evolution encoding.
...
\begin{frame}
  ...
  \framesubtitle{Access to Fourier coefficients}
  
  \footnotesize
  What about the coefficients $c_{n_0,\dots n_{N-1}}\left( \theta \right)$?
  \begin{itemize}
    \item All the elements of the circuit model determine them (data encoding, ansatz, measurement).
    \item No matter the frequency that we encode for $x_i$, if some $c_{n_i}=0$, then we cannot \emph{access} those frequencies.
    \item How each element affects each coefficient is a matter of active research.
  \end{itemize}

  \centering
  \includegraphics[width=.6\textwidth]{pics/coefficients-access.png}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \emph{Problem-agnostic} ansatze suffer from BPs with high $n$ and high depths
  \end{itemize}
