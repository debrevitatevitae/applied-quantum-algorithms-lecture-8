...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Classical operations
    \item When we have an operation we can compute $y\left( \theta \right)$ and $\frac{\partial y}{\partial \theta}$
    \item To compute $\frac{\partial L}{\partial \theta}$ we follow the computational graph \emph{backward} until we get to $\theta$ (chain rule)
    \item In one fwd pass and one bwd pass we have the derivative
    \item This is different than QNNs, which use the PSR and every derivative takes two passes ($\pm \pi/2$), times the number of shots.
  \end{itemize}
